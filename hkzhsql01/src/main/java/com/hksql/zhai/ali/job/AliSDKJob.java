package com.hksql.zhai.ali.job;

import com.hksql.zhai.publicDao.hkRResult.HkRResultService;
import com.hksql.zhai.rStatistics.rStatisticsImg.HKRStatInfoImgService;
import com.hksql.zhai.rStatistics.rStatisticsInfo.RStatisticService;
import com.hksql.zhai.rStatistics.rStatisticsUpdate.HKRStatInfoUpdService;
import com.hksql.zhai.utils.DateUtil;
import org.quartz.Job;
import org.quartz.JobExecutionContext;
import org.quartz.JobExecutionException;

public class AliSDKJob implements Job {
    private HkRResultService hk = new HkRResultService();
    private RStatisticService rs = new RStatisticService();
    @Override
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        String mydate = new DateUtil().getdayDiff(-8,1);
        hk.submitData("10107",10107,mydate);
        rs.submitData(mydate,10107);
        try {
            Thread.sleep(2000);
        rs.submitData(mydate,0);

            Thread.sleep(3000);

        new HKRStatInfoImgService().initType(mydate);

            Thread.sleep(3000);

        new HKRStatInfoUpdService().dataLoad(mydate);

        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
}
